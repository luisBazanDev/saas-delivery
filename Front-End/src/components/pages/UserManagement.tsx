...
export default function UserManagement() {
  ...
  function resetForm() {
    setForm({ name: '', password: '', email: '', role_name: availableRoles[0] || 'STORE_MANAGER', store_id: undefined })
    setEditing(null)
    setShowForm(false)
  }
  ...
  function handleEdit(user: User) {
    setEditing(user)
    setForm({ name: user.name, password: '', email: user.email || '', role_name: user.role_name, store_id: user.store_id })
    setShowForm(true)
  }
          ...
          <form onSubmit={handleSubmit} className="mb-5 p-4 bg-bg-elevated rounded-lg border border-border">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full bg-bg-base border border-border px-3 py-2.5 rounded-lg text-text-primary outline-none focus:border-accent text-[14px]" required />
              </div>
              <div>
              ...
              <tbody>
                {users.map((user) => (
                  <tr key={user.id} className="hover:bg-bg-elevated transition-colors">
                    <td className="py-4 px-3 text-[14px] font-medium border-b border-border">{user.name}</td>
                    <td className="py-4 px-3 text-[14px] border-b border-border">
                      <span className="px-2 py-1 rounded text-[11px] border border-border text-text-secondary">{user.role_name}</span>
                    </td>
